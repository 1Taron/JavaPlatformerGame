...
public class EnemyManager {
	...
	private BufferedImage[][] crabbyArr, pinkstarArr, sharkArr, j_skullArr;
	...
	public void draw(Graphics g, int xLvlOffset) {
		...
		j_drawSkull(g,xLvlOffset);
	}

	//JUR
	private void j_drawSkull(Graphics g, int xLvlOffset) {
		for (JUR_Skull js : currentLevel.getJURSkull())
			if (js.isActive()) {
				g.drawImage(j_skullArr[js.getState()][js.getAniIndex()], (int) js.getHitbox().x - xLvlOffset - JUR_SKULL_DRAWOFFSET_X,
						(int) js.getHitbox().y - JUR_SKULL_DRAWOFFSET_Y + (int) js.getPushDrawOffset(), (int)(JUR_SKULL_WIDTH/2), (int)(JUR_SKULL_HEIGHT/2), null);
//				js.drawHitbox(g, xLvlOffset);
//				js.drawAttackBox(g, xLvlOffset);
			}
	}
...
//				c.drawAttackBox(g, xLvlOffset);
		...
		for (JUR_Skull js : currentLevel.getJURSkull())
			if (js.isActive()) {
				if (js.getState() != DEAD && js.getState() != HIT)
					if (attackBox.intersects(js.getHitbox())) {
						js.hurt(100);
						return;
					}
			}
	...
	private void loadEnemyImgs() {
		...
		//JUR
		j_skullArr = getImgArr(LoadSave.GetSpriteAtlas(LoadSave.JUR_BOSS_SKULL_ATLAS), 9, 5, JUR_SKULL_WIDTH_DEFAULT, JUR_SKULL_HEIGHT_DEFAULT);
	}
	...
	public void resetAllEnemies() {
		...
		for (Shark s : currentLevel.getSharks())
			s.resetEnemy();
		//JUR
		for (JUR_Skull s : currentLevel.getJURSkull())
			s.resetEnemy();
	}
