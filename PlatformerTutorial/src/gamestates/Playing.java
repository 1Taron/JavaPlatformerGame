...
public class Playing extends State implements Statemethods {
	...
	//-----------------------------------
	// JUR
	private BufferedImage j_spacebarUIImgs[], j_arrawUIImgs[], j_mouseUIImgsL[], j_mouseUIImgsR[];
	private boolean j_DrawTutoUI = true;
	private int j_spaceTick, j_arrawTick, j_mouseTickL, j_mouseTickR;
	private int j_spaceAni, j_arrawAni, j_mouseAniL, j_mouseAniR;
	...
	public Playing(Game game) {
		...
		// JUR
		j_spacebarUIImgs = new BufferedImage[3];
		BufferedImage temp_jspace = LoadSave.GetSpriteAtlas(LoadSave.JUR_TUTO_SPACE);
		for (int i = 0; i < j_spacebarUIImgs.length; i++)
			j_spacebarUIImgs[i] = temp_jspace.getSubimage(i * 210, 0, 210, 108);
		j_arrawUIImgs = new BufferedImage[5];
		BufferedImage temp_jarraw = LoadSave.GetSpriteAtlas(LoadSave.JUR_TUTO_ARRAW);
		for (int i = 0; i < j_arrawUIImgs.length; i++)
			j_arrawUIImgs[i] = temp_jarraw.getSubimage(i * 220, 0, 220, 108);
		j_mouseUIImgsL = new BufferedImage[2];
		BufferedImage temp_jmouseL = LoadSave.GetSpriteAtlas(LoadSave.JUR_TUTO_MOUSE_L);
		for (int i = 0; i < j_mouseUIImgsL.length; i++)
			j_mouseUIImgsL[i] = temp_jmouseL.getSubimage(i * 220, 0, 220, 142);
		j_mouseUIImgsR = new BufferedImage[2];
		BufferedImage temp_jmouseR = LoadSave.GetSpriteAtlas(LoadSave.JUR_TUTO_MOUSE_R);
		for (int i = 0; i < j_mouseUIImgsR.length; i++)
			j_mouseUIImgsR[i] = temp_jmouseR.getSubimage(i * 220, 0, 220, 142);
	...
	public void update() {
		if (paused)
			...
			// JUR
			if (j_DrawTutoUI) {
				j_updateSpcaeTutoAni();
				j_updateArrawTutoAni();
				j_updateMouseLTutoAni();
				j_updateMouseRTutoAni();
			}
		}
	}
	
	// JUR
	private void j_updateArrawTutoAni() {
		j_arrawTick++;
		if (j_arrawTick >= 27) {
			j_arrawTick = 0;
			j_arrawAni++;
	...
	private void j_updateSpcaeTutoAni() {
		j_spaceTick++;
		if (j_spaceTick >= 35) {
			j_spaceTick = 0;
			j_spaceAni++;
	...
	private void j_updateMouseLTutoAni() {
		j_mouseTickL++;
		if (j_mouseTickL >= 35) {
			j_mouseTickL = 0;
			j_mouseAniL++;
	...
	private void j_updateMouseRTutoAni() {
		j_mouseTickR++;
		if (j_mouseTickR >= 35) {
			j_mouseTickR = 0;
			j_mouseAniR++;
	...
	//JUR
	private void j_DrawTutoUIImgs(Graphics g) {
		//Arraw
		g.drawImage(j_arrawUIImgs[j_arrawAni], (int) (300 * Game.SCALE) - xLvlOffset, (int) ((180 * Game.SCALE)),
				(int) (220 * Game.SCALE) / 2, (int) (108 * Game.SCALE) / 2, null);
		//Spacebar
		g.drawImage(j_spacebarUIImgs[j_spaceAni], (int) (780 * Game.SCALE) - xLvlOffset, (int) ((150 * Game.SCALE)),
				(int) (210 * Game.SCALE) / 2, (int) (108 * Game.SCALE) / 2, null);
		//Mouse
		g.drawImage(j_mouseUIImgsL[j_mouseAniL], (int) (1200 * Game.SCALE) - xLvlOffset, (int) ((120 * Game.SCALE)),
				(int) (220 * Game.SCALE) / 2, (int) (142 * Game.SCALE) / 2, null);
		g.drawImage(j_mouseUIImgsR[j_mouseAniR], (int) (2110 * Game.SCALE) - xLvlOffset, (int) ((120 * Game.SCALE)),
				(int) (220 * Game.SCALE) / 2, (int) (142 * Game.SCALE) / 2, null);
	}
	...
	public void JCcheckObjectHit(Rectangle2D.Float attackBox) {
		objectManager.JCcheckObjectHit(attackBox);
	}
